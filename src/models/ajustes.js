...
class Ajustes {
  servirAjustes = async (req, res) => {
    try {
      ...
      const resAjustes = await pool.query('CALL listarAjustes')
      const ajustes = JSON.stringify(resAjustes[0][0])
  ...
  encriptarArchivo = (req, res) => {
    try {
      const rutaSalida = path.join(__dirname, '../../', 'cuentabytesEncriptado.sql')
