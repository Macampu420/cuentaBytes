const modalProvee = new bootstrap.Modal(document.getElementById('modalProvee'))
...
document.getElementById('btnEliminar').addEventListener('click', async event => {
  const conf = confirm('¿Desea eliminar el proveedor?')
...
document.getElementById('buscadorProvee').addEventListener('change', () => {
  const proveedores = document.querySelectorAll('[cartaItem]')
  let swNoCoinc = false
  ...
  proveedores.forEach(element => {
    const nombreProvee = element.firstElementChild.innerHTML.toLowerCase()
