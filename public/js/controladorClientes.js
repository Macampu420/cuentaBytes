const modalReg = new bootstrap.Modal(document.getElementById('modalCl'))
...
document.getElementById('btnEliminar').addEventListener('click', async event => {
  const conf = confirm('¿Desea eliminar El Cliente?')
...
document.getElementById('buscadorClientes').addEventListener('change', () => {
  const clientes = document.querySelectorAll('[cartaItem]')
  let swNoCoinc = false
  ...
  clientes.forEach(element => {
    const nombreCliente = element.firstElementChild.innerHTML.toLowerCase()
