const vObjsTipoEgreso = []
const vObjsVentaActual = []
let vItemsEgreso = []
let numeroItem = 0
let numeroItemAct = 0
const vObjsEncEgreso = []
let vItemsEditar = []
...
const vrTotalRegistar = () => {
  ...
  for (let i = 0; i < numeroItem; i++) {
    const valorEgreso = parseInt(document.getElementById(`vr${i}`).value)
...
const detalleVenta = (vectorId) => {
  ...
  for (let i = 0; i < numeroItem; i++) {
    const valorEgreso = parseInt(document.getElementById(`vr${i}`).value)
    const descripcion = document.getElementById(`titulo${i}`).value
    acumEgreso = acumEgreso + valorEgreso
    let idDetEgreso = 0
    ...
    if (vectorId.length > 0) {
      if (i < numeroItemAct) {
        ...
        // }
        const idDetEgreso = parseInt(document.getElementById(`id${i}`).className)
...
const registrarEgreso = (vector) => {
  const tituloEgreso = document.getElementById('inpTitulo').value
  const fecha = document.getElementById('inpFecha').value
  const idTipoEgreso = parseInt(document.getElementById('slcTipoEgreso').value)
  let vrTotalEgreso = 0
...
const renderEgresos = async () => {
  ...
  if (vObjsEncEgreso.length > 0) {
    const filaEgresos = document.getElementById('filaEgresos')
...
const modalEditar = async event => {
  ...
  const idVenta = event.target.getAttribute('idventa')
