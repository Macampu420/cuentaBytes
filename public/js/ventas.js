const vObjsProductos = []
let vObjAjustes
const vObjsCliente = []
const vObjsVentaActual = []
let vItemsVta = []
let numeroItem = 0
const stockActual = 0
const nuevoStock = 0
const vObjsEncVentas = []
const vItemsEditar = []
const vItemsElim = []
...
const traerAjustesVta = async () => {
  ...
  const resAjustes = await fetch('http://localhost:3000/ajustes')
...
const traerMetodosPago = async () => {
  try {
    const resMetodosPago = await fetch('http://localhost:3000/listarMetodoPago')
...
const vrTotalRegistar = (vItems) => {
  ...
  const dto = document.getElementById('inpDescuento').value
  let vrTot = 0
...
const actualizarCrearItem = (item, disparador, vector) => {
  const nroItemDisparador = parseInt(disparador.id.match(/\d+$/)[0])
  const productoItem = vObjsProductos.find(producto => producto.idProducto == disparador.value)
...
const actualizarUnidadesVendidas = (disparador, vector) => {
  const nroIdItem = parseInt(disparador.id.match(/\d+$/)[0])
...
const registrarVenta = () => {
  const idMetPago = document.getElementById('slcMetodoPago').value
  const idCliente = parseInt(document.getElementById('slcClientes').value)
  const descuento = parseInt(document.getElementById('inpDescuento').value)
...
const renderVentas = async () => {
  ...
  if (vObjsEncVentas.length > 0) {
    const filaVentas = document.getElementById('filaVentas')
...
const iniciarModalDetallesVenta = async (disparador) => {
  const resVenta = await fetch(`http://localhost:3000/editVenta${disparador.getAttribute('idVenta')}`)
