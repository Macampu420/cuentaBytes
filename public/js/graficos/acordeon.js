...
export default class modeloGraficos {
  ...
  traerDatosProductos = async (inicio, fin, tiempo) => {
    try {
      if (tiempo == 'horas') {
        const resDatosProductos = await fetch(`${this.#urlBaseNode}/reportesProductos/${tiempo}`, {
          method: 'POST',
        ...
        const datosProductos = await resDatosProductos.json()
  ...
  mostrarDatosProductos = (vectorDatos, tipoBalance, propiedadData, labelGrafico) => {
    ...
    const nombresProductos = vectorDatos[tipoBalance].map(elemento => elemento.nombreProducto)
  ...
  traerDatosVentas = async (inicio, fin, tiempo) => {
    try {
      if (tiempo == 'horas') {
        const resDatosVentas = await fetch(`${this.#urlBaseNode}/reportesVentas/${tiempo}`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: {
            'Content-Type': 'application/json'
        ...
        const datosVentas = await resDatosVentas.json()
        return datosVentas
      } else {
        const resDatosVentas = await fetch(`${this.#urlBaseNode}/reportesVentas/${tiempo}`, {
          method: 'POST',
  ...
  mostrarDatosVentas = (vectorDatos, tipoBalance, propiedadData, propiedadLabels, labelGrafico) => {
    ...
    // se obtienen todos los valores de las horas correspondientes
    const horaVentas = vectorDatos[tipoBalance].map(elemento => elemento[propiedadLabels])
  ...
  traerDatosCompras = async (inicio, fin, tiempo) => {
    try {
      if (tiempo == 'horas') {
        const resDatosCompras = await fetch(`${this.#urlBaseNode}/reportesCompras/${tiempo}`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: {
            'Content-Type': 'application/json'
        ...
        const datosCompras = await resDatosCompras.json()
        return datosCompras
      } else {
        const resDatosCompras = await fetch(`${this.#urlBaseNode}/reportesCompras/${tiempo}`, {
          method: 'POST',
  ...
  mostrarDatosCompras = (vectorDatos, tipoBalance, propiedadData, propiedadLabels, labelGrafico) => {
    ...
    // se obtienen todos los valores de las horas correspondientes
    const horaCompras = vectorDatos[tipoBalance].map(elemento => elemento[propiedadLabels])
  ...
  traerDatosEgresos = async (inicio, fin, tiempo) => {
    try {
      if (tiempo == 'horas') {
        const resDatosEgresos = await fetch(`${this.#urlBaseNode}/reportesEgresos/${tiempo}`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: {
            'Content-Type': 'application/json'
        ...
        const datosEgresos = await resDatosEgresos.json()
        return datosEgresos
      } else {
        const resDatosEgresos = await fetch(`${this.#urlBaseNode}/reportesEgresos/${tiempo}`, {
          method: 'POST',
  ...
  mostrarDatosEgresos = (vectorDatos, tipoBalance, propiedadData, propiedadLabels, labelGrafico) => {
    ...
    // se obtienen todos los valores de las horas correspondientes
    const nombresCliente = vectorDatos[tipoBalance].map(elemento => elemento[propiedadLabels])
  ...
  traerDatosClientes = async (inicio, fin, tiempo) => {
    try {
      if (tiempo == 'horas') {
        const resDatosClientes = await fetch(`${this.#urlBaseNode}/reportesClientes/${tiempo}`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: {
            'Content-Type': 'application/json'
        ...
        const datosClientes = await resDatosClientes.json()
