...
const seccionesAcordeon = [
  {
    ...
    callbackFecha: async function (pInicio, pFin) {
      const inicio = pInicio.format('YYYY-MM-DD')
      const fin = pFin.format('YYYY-MM-DD')

      const fechaSeleccionada = pInicio.clone().startOf('day')
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  const graficoClientes = new Chart(document.getElementById('canvaClientes'), conficInicialGraficos.configClientes)
  const graficoEgresos = new Chart(document.getElementById('canvaEgresos'), conficInicialGraficos.configEgresos)
  const graficoVentas = new Chart(document.getElementById('canvaVentas'), conficInicialGraficos.configVentas)
  const graficoCompras = new Chart(document.getElementById('canvaCompras'), conficInicialGraficos.configCompras)
  const graficoProductos = new Chart(document.getElementById('canvaProductos'), conficInicialGraficos.configProductos)
  ...
  document.getElementById('containerDropdownProductos').addEventListener('click', event => {
    const tipoBalance = objModeloGraficos.convertirACamelCase(event.target.id.replace(/-/g, ' '))
