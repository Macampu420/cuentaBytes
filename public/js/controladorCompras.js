const divModal = document.getElementById('modalCompras')
const modalCompras = new bootstrap.Modal(divModal)
...
document.getElementById('tblItemsVta').addEventListener('input', (event) => {
  ...
  if (event.target.id.includes('inpCantidad')) {
    ...
    if (event.target.value > parseInt(event.target.getAttribute('max'))) { event.target.value = parseInt(event.target.getAttribute('max')) }
...
document.getElementById('buscadorCompras').addEventListener('change', () => {
  const compras = document.querySelectorAll('[cartaItem]')
  let swNoCoinc = false
  ...
  compras.forEach(element => {
    const conceptoCompra = element.firstElementChild.innerHTML.toLowerCase()
